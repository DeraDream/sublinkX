...
- 节点列表新增独立的主控延迟检测，不改动家宽 Agent 测速逻辑
- 主控测速同时展示总延迟、主控到节点入口延迟以及入口后的代理出口延迟
- 主控测速结果持久化保存，支持测试选中节点或当前列表全部节点

## [3.2更新说明]

- 修复家宽 Agent 延迟长期显示 `1 ms` 的问题
- 延迟改为通过节点代理访问 HTTP 204 探测地址，预热后使用独立连接多次测试并取最低成功值

## [3.1更新说明]

- 家宽 Agent 仅在版本低于当前版本时显示更新命令
- 家宽节点延迟改为家庭设备到节点入口的多次 TCP 探测，避免公网探测站延迟干扰
- 家宽下载测速改为多连接并发聚合，减少单连接对测速结果的低估
